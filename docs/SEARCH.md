...
| `synthesize` | `bool` | `True` | `False` пропускает финальный large-model синтез. Предшествующий rerank **по-прежнему выполняется** — его результат теперь и есть `sources` (best-first), а не только вход синтеза. `answer` возвращается `""`; **`sources`, `documents` и `step_stats` не меняются**. Для клиента, который сам собирает ответ из `sources` и не хочет платить за синтез |
...
чекпойнта ~1 ГБ. Оркестратор запрашивает `top_n=len(merged)` — **весь**
объединённый пул, а не top-N; `TEMPORAL_RERANK_TOP_N` (`rerank_top_n`, по
умолчанию 5) больше не задаёт размер этого запроса — он ограничивает
только то, что попадает в синтез (см. ниже). Выполняется на малой очереди
поиска.

- **Успех**: `SearchOutcome.sources` — это ранжированный результат rerank
  (best-first), весь пул без потерь (для пулов больше внутреннего
  `_RERANK_SCORE_CAP` cross-encoder'а — ранжированная голова плюс остаток
  пула в исходном порядке, см. `append_unranked_remainder`,
  `activities/rerank.py`). `rerank_top_n` больше не ограничивает то, что
  видит вызывающий — только то, что уходит в синтез (`cap_synth_sources`).
- **Fail-open**: любая ошибка rerank → `sources` откатываются к
  объединённому пулу БЕЗ ранжирования (порядок ретривера), но так же
  полностью — эта деградация не изменилась этой веткой. Контекст синтеза
  при этом всё равно ограничивается `rerank_top_n` (чистый
  `cap_synth_sources`) — неограниченный откат мог бы превысить таймаут
  start_to_close у `synthesize_answer`.
- **`synthesize=False` → rerank теперь ВСЕГДА выполняется.** Раньше блок
  rerank жил внутри `if params.synthesize:`, и единственным потребителем
  его результата был `build_synthesize_call` — при `synthesize=False`
  rerank пропускался как ненужная работа, и это было незаметно клиенту,
  поскольку `sources` и так был необработанным объединённым пулом. Теперь
  `sources` — это сам результат rerank, поэтому пропускать его больше
  нельзя: `synthesize=False` возвращает те же ранжированные источники,
  просто без синтеза поверх них. Инвариант «`sources` одинаковы при
  `synthesize=True` и `False`» по-прежнему закреплён тестом
  `test_outcome_sources_identical_with_and_without_synthesis` — теперь
  потому что rerank выполняется ОДИН раз до ветвления на `synthesize`, а не
  потому что `sources` обходит rerank.
...
`SearchOutcome` возвращает ответ, весь пул источников — ранжированный
rerank'ом (best-first), без потерь — различные `doc_ids`
(`distinct_doc_ids`, пропуская частичные результаты сообществ),
статистику по шагам, цитаты, неопределённости и латентность.
...
| `rerank_sources` | `sources` = объединённый пул БЕЗ ранжирования, полностью; контекст синтеза всё равно ограничен `rerank_top_n` (деградация) |
