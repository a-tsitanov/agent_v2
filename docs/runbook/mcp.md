...
> синтез — `answer` возвращается `""`. Предшествующий rerank **по-прежнему
> выполняется**, при `True` и при `False` одинаково: его ранжированный
> результат теперь и есть `sources` (best-first), а не только вход синтеза,
> так что его больше нельзя пропустить — читает каждый клиент. Смысл:
> клиент, который сам собирает ответ из `sources`, перестаёт платить
> (временем и токенами large-tier модели) только за сам синтез. По
> умолчанию `True` — существующие клиенты поведения не меняют.
>
