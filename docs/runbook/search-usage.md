...
синтез на любом из четырёх эндпоинтов: `answer` приходит `""`, `sources` и
`documents` — как обычно. **rerank при этом всё равно выполняется** —
`sources` приходит best-first (отранжированным), а не в произвольном
порядке слияния, ровно как и при `synthesize: true`; это ровно тот случай,
где порядок важнее всего, потому что клиент сам читает `sources` вместо
того, чтобы получить готовый ответ. Для клиента, который сам пишет ответ по
чанкам и не хочет платить за большой синтез задержкой и токенами. По
умолчанию `true` — существующие вызовы не меняются. Полный контракт (что
именно возвращается пустым) —
